...
void seal2tfhe::ciphertext_seal_exit(TLweSample  * tlweCipher, seal::Ciphertext sealCipher, const seal::SEALContext &sealcontext)
{
  ...
  auto &context_data = *sealcontext.key_context_data();
  auto coeff_modulus = context_data.parms().coeff_modulus();
  // Use the total modulus for fresh ciphertexts
  //uint64_t q = context_data.total_coeff_modulus()[coeff_modulus.size() - 1];
  uint64_t q = coeff_modulus[0].value();
  //cout << "Total modulus q: " << q << endl;
  int N_seal = context_data.parms().poly_modulus_degree();
  int N_tfhe = 4096 ;
  ...
  if (N_seal < N_tfhe) {
    std::cerr << "ERROR: N_seal (" << N_seal << ") < N_tfhe (" << N_tfhe << ")" << std::endl;
    return;
  }

  uint64_t * c2 = sealCipher.data(1);
  if (c2 == nullptr) {
    std::cerr << "ERROR: sealCipher.data(1) returned nullptr" << std::endl;
    return;
  }
  
  // Use vector instead of malloc for automatic memory management
  std::vector<uint32_t> c1(N_tfhe);
  ...
  // c1 is automatically freed here when vector goes out of scope
}
...
void seal2tfhe::secretKey(TLweKey * tlweKey, seal::SecretKey sealKey, const seal::SEALContext &sealContext)
{
  ...
  auto coeff_modulus = context_data.parms().coeff_modulus();
  uint64_t q = coeff_modulus[0].value();
  cout << "Inside Seal key to TFHE key, Total modulus q: " << q << endl;
  int N_tfhe = 4096;
  ...
  for (size_t i = 0; i < N_tfhe; i++)
  {
    key_coefs[i] %= q;  // Ensure it's reduced mod q
    if (key_coefs[i] == q-1)
      key_coefs[i] = -1;
