...
FILE * tgsw_bparams_file = fopen("../data/System_Parameters/params_tgsw_b.txt" , "r");
...
TGswParams * b_tgsw_params = new_tGswParams_fromFile(tgsw_bparams_file);
...
TGswKey * b_tgsw_key = new_TGswKey(b_tgsw_params);
seal2tfhe::secretTGSWKey(b_tgsw_key, init_seal_key, context);
//tGswKeyGen(b_tgsw_key);
cout << "TGSW key generated" << endl;
TLweKey * b_tlwe_key = &(b_tgsw_key->tlwe_key);
LweKey * b_lwe_key = new_LweKey(b_lwe_params);
tLweExtractKey(b_lwe_key, b_tlwe_key);  // This is the key that will be used to generate the bootstrapping key. Otherwise it is not used anywhere else. 
...
cout << "Lwe key extracted" << endl;
const int switch_t = 8;//7
const int switch_base = 2;//4 
LweKeySwitchKey * ks_med_key = new_LweKeySwitchKey(4096, switch_t, switch_base, init_lwe_params);
lweCreateKeySwitchKey(ks_med_key, b_lwe_key, init_lwe_key);
clock_t end_usr_kgen2 = clock();

cout << "Bridge to init KSK made" << endl;
...
cout << "Init to Out KSK made" << endl;
// cout << "b_lwe_key n: " << b_lwe_key->params->n << endl;
// cout << "init_lwe_key n: " << init_lwe_key->params->n << endl;
// cout << "out_lwe_params n: " << out_lwe_params->n << endl;
// cout << "out_tgsw_params N: " << out1_tgsw_key->params->tlwe_params->N << endl;
// cout << "out1_tgsw_key->params->l: " << out1_tgsw_key->params->l << endl;
// cout << "out1_tgsw_key->params->Bgbit: " << out1_tgsw_key->params->Bgbit << endl;
// Bootstrapping key generation; Unique to each user.


clock_t start_usr_kgen4 = clock();
LweBootstrappingKey * boot_key = new_LweBootstrappingKey(switch_t , switch_base, init_lwe_params, out1_tgsw_key->params);
// cout << "Bootstrapping key generation started" << endl;
// cout << "Verifying parameter consistency:" << endl;
// cout << "boot_key->bk_params == out1_tgsw_key->params: " << (boot_key->bk_params == out1_tgsw_key->params ? "YES" : "NO") << endl;
// cout << "boot_key->in_out_params == init_lwe_key->params: " << (boot_key->in_out_params == init_lwe_key->params ? "YES" : "NO") << endl;
// cout << "boot_key->bk_params->tlwe_params->N: " << boot_key->bk_params->tlwe_params->N << endl;
// cout << "out1_tgsw_key->params->tlwe_params->N: " << out1_tgsw_key->params->tlwe_params->N << endl;
// cout << "boot_key->in_out_params->n: " << boot_key->in_out_params->n << endl;
// cout << "init_lwe_key->params->n: " << init_lwe_key->params->n << endl;
    // const LweParams *in_out_params = boot_key->in_out_params;
    // const TGswParams *bk_params = boot_key->bk_params;
    // const TLweParams *accum_params = bk_params->tlwe_params;
    // const LweParams *extract_params = &accum_params->extracted_lweparams;
// cout << "Parameters for bootstrapping key generation set" << endl;
// cout << "TGsw TLWE key pointer: " << out1_tgsw_key->tlwe_key.key << endl;
//     //LweKeySwitchKey* ks; ///< the keyswitch key (s'->s)
    // const TLweKey *accum_key = &out1_tgsw_key->tlwe_key;
//     cout << "Accum key set" << endl;
//     cout << "Generating bootstrapping key using library function" << endl;
    // LweKey *extracted_key = new_LweKey(extract_params);
    // tLweExtractKey(extracted_key, accum_key);
    // lweCreateKeySwitchKey(boot_key->ks, extracted_key, init_lwe_key);
    // delete_LweKey(extracted_key);
    // int32_t *kin = init_lwe_key->key;
    ...
    //     tGswSymEncryptInt(&boot_key->bk[i], kin[i], alpha, out1_tgsw_key);
    // }
    tfhe_createLweBootstrappingKey(boot_key, init_lwe_key, out1_tgsw_key);
    // cout << "Bootstrapping key generated successfully" << endl;
