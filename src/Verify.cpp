...
{
  if (argc < 3) {
    std::cerr << "Usage: " << argv[0] << " <threshold_value>"  << " <Test Biometric file> " << " <Original Stored Biometric> " << std::endl;
    return 1;
  }
...
dataIO::Read_F("../data/E-Gate/"+std::string(argv[2]), context, testSEALCipher, testSEALCipherSquare, testSEALCosineCipher);

Ciphertext modelSEALCipher; 
Ciphertext modelSEALCipherSquare; 
Ciphertext modelSEALCosineCipher;

dataIO::Read_F("../data/CountryDB/"+std::string(argv[3]), context, modelSEALCipher, modelSEALCipherSquare, modelSEALCosineCipher);
