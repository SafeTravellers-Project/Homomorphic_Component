...
{
    if (argc < 2) {
        std::cerr << "Usage: " << argv[0]  << " <Input Raw Test Biometric folder>"  << " Output Encrypted Test Biometric folder> " << std::endl;
        return 1;
    }
    srand(time(0));
...
for (const auto & fileName_model_f : biometric_files) {
    ...
    dataIO::EncryptF2F(fileName_model_to, fullName_model_f, init_encryptor, vector_size, precision, p, 1);
