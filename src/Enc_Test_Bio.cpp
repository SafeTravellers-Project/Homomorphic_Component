...
int main(int argc, char *argv[])
{
  try {
    if (argc < 3) {
      ...
      << " <Input Raw Test Biometric folder> <Output Encrypted Test Biometric folder>\n";
      return 1;
    }
    ...
    if (vector_size > N_seal) {
      std::cerr << "The poly_modulus_degree must be >= biometric vector size\n";
      return 1;
    }
    ...
    if (ec) {
      std::cerr << "Error: Cannot create output folder: " << output_dir
      << " (" << ec.message() << ")\n";
      return 1;
    }
    ...
    for (const auto &item : biometric_files) {
      ...
      // Create output subdir per input file
      fs::path out_bio_dir = output_dir / (item.name + "_enc");
      fs::create_directories(out_bio_dir, ec);
      if (ec) {
        throw std::runtime_error("Cannot create output directory: " + out_bio_dir.string() +
        " (" + ec.message() + ")");
      }
      ...
      // IMPORTANT: pass the FULL PATH of the embedding file (not just basename)
